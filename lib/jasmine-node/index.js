...
jasmine.setupCoverage = function(options){
    ...
    if(options.cover){
        ...
        var dest = source + '-cov';
        coverage.processFile(source, dest, options.excludes);
        ...
        jasmineEnv.addReporter(new EmmaReporter('.', options.savePath, function(){
            var covershot = require('covershot');
            covershot.writeCoverage(options.savePath);
            covershot.generate({ _: [ options.savePath + 'data' ],
                f: 'html',
                write: options.savePath + 'covershot/'
            }, function(){
                console.log('Finished Creating Covershot HTML at', options.savePath + 'covershot/');
                rmdir.sync(options.savePath + 'data');
                if(options.done ){
                    options.done();
                }
                else if(options.coverageForceExit){
                    process.exit(0);
                }
            });
            totalCoverage();
...
function totalCoverage() {
    ...
    for (var i in _$jscoverage) {
        ...
        allTotal += total ? total : 0;
        allTouched += touched ? touched : 0;
    }
    var totalPercent = allTouched / allTotal;
    var totalResult = totalPercent.toFixed(2) * 100;
    var color = 'GREEN';
    if(totalResult < 80){
        color = 'YELLOW';
    }
    ...
    console.log(colorful(color, '\n[JSCOVERAGE] Total coverage ' + totalResult + '%' ));
};

function colorful(type, str) {
