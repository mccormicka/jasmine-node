...
while(args.length) {
  ...
  switch(arg)
  {
    ...
    case '--config':
        ...
        break;
    case '--coverage':
        doCoverage = args.shift();
        if(!existsSync(doCoverage))
            throw new Error("source coverage directory '" + doCoverage + "' doesn't exist!");
      break;
    case '--coverage-excludes':
      coverageExcludes = args.shift();
      break;
    case '-h':
        help();
      break;
...
if(doCoverage){
    ...
    var source = doCoverage;
    var dest = source + '-cov';
    var excludes = [];
    if(coverageExcludes){
        var glob = require('glob');
        var tempExcludes = glob.sync(coverageExcludes);
    ...
    jasmineEnv.addReporter(new EmmaReporter('.', options.junitreport.savePath, function(coverage){
