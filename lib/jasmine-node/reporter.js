(function() {
  ...
  jasmineNode.TerminalReporter.prototype = {
    ...
    ANSIColors: {
        ...
        skipped: function() { return '\033[33m'; }, // Yellow
    ...
    printRunnerResults_: function(runner){
      ...
      msg += this.stringWithColor_( results.failedCount + ' failure' + ((results.failedCount === 1) ? '' : 's') + '\n',
          (results.failedCount > 0 ? this.color_.fail() : this.color_.pass()) );
      msg += this.stringWithColor_( results.skippedCount + ' skipped assertions' , (results.skippedCount > 0 ? this.color_.skipped() : this.color_.pass()));
        return msg;
    },
  ...
  jasmineNode.TerminalVerboseReporter.prototype = {
    reportSpecResults: function(spec) {
      ...
      this.specResults_[spec.id] = {
        ...
        result: spec.results().failedCount > 0 ? 'failed' : spec.results().skippedCount > 0 ? 'skipped' : 'passed'
      };
    },
      ...
      for (var i = 0; i < results_length; i++) {
        ...
        if (element.type === 'spec') {
          ...
          if( specResult){
              ...
              }else if(specResult.result === 'skipped'){
                  msg = this.stringWithColor_(this.indentMessage_(element.name, depth), this.color_.skipped());
              } else {
